...
func _ready():
	...
	_cost_label.text = str(CardRepo.buffs[card_id]["cost"])
	_name_label.text = CardRepo.buffs[card_id]["name"]
	_health_label.text = str(CardRepo.buffs[card_id]["health"])
	_icon_sprite.texture = load(CardRepo.buffs[card_id]["icon"])
	...
	if(_is_dragging):
		...
		for a in get_overlapping_areas():
			if(a.is_in_group("Field")):
				if(a.has_seed):
					if(!a.is_harvestable()):
						if(GameStage.remove_money(CardRepo.buffs[card_id]["cost"])):
							_should_return = false
							a.heal(CardRepo.buffs[card_id]["health"])
							_is_consumed = true
