...
onready var _seed_card_scene = load("res://Common/Res/Cards/SeedCard.tscn")
...
func buy_card():
	if(HandManager.get_card_count() < 6):
		if(GameStage.remove_money(1)):
			spawn_random_card()

func spawn_random_card():
	var pull_list = []
	for b in CardRepo.seeds:
...
func spawn_card(id:int):
	var card = _seed_card_scene.instance()
	card.card_id = id
...
func _on_SeedDeck_input_event(viewport, event, shape_idx):
...
func _on_SeedDeck_mouse_entered():
	_mouse_over = true


func _on_SeedDeck_mouse_exited():
	_mouse_over = false
